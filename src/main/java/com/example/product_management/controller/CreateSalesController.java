...
//        Connection conn = getConnection();
//        try (PreparedStatement stmt = conn.prepareStatement("SELECT price FROM products WHERE name = ?")) {
//            stmt.setString(1, productName);
//            try (ResultSet rs = stmt.executeQuery()) {
//                if (rs.next()) {
...
//        });
//
//        new Thread(task).start(); // ✅ এখন task চালু হবে
//    }
//
//    @FXML
...
//                    int totalQty = item.getQuantity() + requestedQty;
//                    if (totalQty > availableStock) {
//                        showAlert("Stock Error", "Only " + availableStock + " item(s) available including what's already in cart.");
//                        return;
//                    }
//                    item.setQuantity(totalQty);
//                    item.setSubtotal(totalQty * item.getPrice());
//                    cartTable.refresh();
//                    updateTotal();
//                    quantityField.clear();
...
//            }
//
//            cartItems.add(new CartItem(product, requestedQty, price));
//            updateTotal();
//            quantityField.clear();
//            productComboBox.getSelectionModel().clearSelection();
//            productComboBox.getEditor().clear();
...
//        if (stage == null) {
//            stage = (Stage) productComboBox.getScene().getWindow();
//        }
//        try {
...
public class CreateSalesController {
    ...
    private void saveDraft() {
        if (currentCustomer != null) {
            draftManager.saveDraft(currentCustomer.getPhone(), new CartMemento(List.copyOf(cartItems)));
        }
    }
